...
export default function MembersHub() {
  const { isAuthenticated, isLoading, user, hasAccessToProduct, loginWithEmail, sendMagicLink, verifyCode, emailAccess, logout } = useMemberAccess();
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      if (result.sent) {
        ...
        setEmailError('If you have purchases with this email, you will receive a verification code.');
      }
    } else {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    const result = await verifyCode(emailInput, codeInput);
    if (!result.success) {
  ...
  // Legacy direct login (for quick access - less secure)
            ...
            {verificationStep === 'email' ? (
              <form onSubmit={handleSendCode}>
                ...
                <button
                  ...
                  className="w-full mt-2 py-2 px-4 text-sm text-gray-500 hover:text-gray-700 transition-colors"
                >
                  Quick access (no verification)
                </button>
              </form>
            ) : (
              <form onSubmit={handleVerifyCode}>
                ...
                <input
                  ...
                  className="w-full text-center text-2xl tracking-widest py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all bg-gray-50"
                  placeholder="000000"
                  maxLength={6}
                  required
                />
                ...
                <button
                  type="submit"
                  disabled={isCheckingEmail || codeInput.length !== 6}
                  className="w-full mt-4 py-3 px-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(251,191,36,0.3)]"
                >
                  {isCheckingEmail ? (
                    <>
